...
カメラ情報（latitude / longitude / zoom / pitch / bearing）は URL の**ハッシュ**（`#latitude=...&longitude=...`）に保持します。ハッシュはアドレスバーで書き換えてもページが再読み込みされないため、座標を変更すると現在のビューからそのまま Google Earth 風（距離に応じた飛行時間＋一度ズームアウトして降下する弧）に FLYTO します。`project` などカメラ以外のパラメータはクエリ（`?project=...`）に保持します。
