...
KASUGAI Canvas の現行実装は、ローカルPC上で Rust/Axum サーバーを起動し、ブラウザ上の CesiumJS と Three.js で地理空間データを描画する構成です。開発起動時の初期インスペクターは空ですが、配布 ZIP と Windows インストーラーには東京都の表示例を含む `kasugai_canvas.config` が初期サンプルとして同梱されます。
...
開発起動時の初期インスペクターは空です。配布物では同梱の `kasugai_canvas.config` が起動時に読み込まれ、XYZ、GeoJSON、3D Tiles、Base map などの東京都の表示例が初期状態で追加されます。設定を登録すると、設定ファイルの内容が更新されます。
...
ここでいう**DEMテクスチャー**は、DEM から作った地形メッシュに Base map 画像を貼る表示です。一方、**ドレープ**は、XYZ・GeoJSON などのレイヤーを DEM または 3D Tiles の地形表面の起伏に沿わせて貼る表示です。前者は地形の下地画像、後者は既存レイヤーの地形追従であり、別の処理です。
...
DEMの選択、Terrain の表示、ドレープ対象地形、ドレープ適用先は、それぞれ独立した設定です。処理は次の順序で決定します。
...
設定タブ内は、DEM 表示・DEM 選択・BASEMAP 専用の 3D Tiles ドレープを `DEM`、ドレープ対象地形・ドレープ適用レイヤー・インスペクターを `レイヤ`、Shadow を `その他` へ分割しています。
...
Terrain を ON にした場合、Base map は `ImageryLayer` として Globe 表面へ描画されます。`3D Tiles (BASEMAPドレープ)` を ON にすると、3D Tiles 表面へ別の `ImageryLayer` 相当のテクスチャを重ねます。DEM テクスチャーと XYZ・GeoJSON の地形追従は CesiumJS の `Globe` ・ `ImageryLayer` ・ `Entity` 挙動で自然に行われます。Terrain を OFF にした場合も、表示中の 3D Tiles があれば Base map を 3D Tiles 表面へドレープし、それ以外は通常の `ImageryLayer` を表示します。
...
ユーザーがテキストエリアに JSON 形式で入力したレイヤー・ベースマップ・カメラプリセットを読み込み、CesiumJS  Viewer へ反映します。設定の読み込みはブラウザ内で完結し、HTTP 通信は行いません。
...
## CesiumJS 移行における注意事項と制限

### 3D Tiles への XYZ ドレープ

CesiumJS 1.130 以降で `Cesium3DTileset.imageryLayers` が追加され、3D Tiles 表面に `ImageryLayer` をドレープできるようになりました。KASUGAI Canvas では 1.143.0 を使用しています。
...
V1.0.0 時点で実装されている主な機能は以下の通りです。

- **レイヤー管理**
  - XYZ / GeoJSON / 3D Tiles レイヤーの追加、表示切替、削除
  - 通常グループ / Exclusive グループ / 排他選択
  - ドラッグ＆ドロップによるレイヤー並び替えと `kasugai_canvas.config` への永続化
- **ベースマップ切替**（OSM / 地理院タイル / 航空写真等）と不透明度設定
- **ナビゲーション**
  - Fly モード / 2D トップダウン
  - 方位コンパス
  - カメラプリセット
- **URL パーマリンク**（latitude / longitude / zoom / pitch / bearing / project）
  - カメラ移動に合わせた自動更新
  - 共有 URL コピー
- **検索**（Yahoo 住所検索 / 国土地理院住所検索 / Nominatim フォールバック）
- **情報・凡例・属性表示**（属性内 URL のハイパーリンク対応）
- **地形・ドレープ**
  - DEM / 3D Tiles ドレープ対象の個別選択
  - XYZ / GeoJSON の地形追従
- **設定**
  - 地下移動 / 地表透過 / 背景色
  - Cesium エフェクト（影、霧、空気、深度テスト、SSE、メモリ等）
  - 3D Tiles のクリッピング（NS / EW / H / 解除）
- **プロジェクト切替**（`projects/<project>` 単位）
- **自動更新**（GitHub Pages `latest.json` 確認、ZIP ダウンロード、再起動）
