...
- **GeoJSON 正規化経路**: 非対応形式をデコーダーで GeoJSON 化して同じ DataSource 経路に乗せる。デコーダーは `web/app.js` の **`vectorDecoders` レジストリ**（type→`async (item) => GeoJSON` の Map）に登録する
  ...
  - 実装済み: `gpkg:` / `geopackage:`（`loadGeoPackageAsGeoJson()`。sql.js(SQLite WASM)を CDN から遅延ロードし、GPKG を SQLite として直接開いて地物テーブルを全件読み込み。GPkgBinary ヘッダを剥がして WKB→GeoJSON ジオメトリに変換。`table=` で複数地物テーブルから対象を選択可。QGIS が「データベースに保存」する `layer_styles` テーブルの QML を読み取り `useAsDefault` を優先して `item.qmlStyle` に適用（`style=` 明示指定が優先）。**DuckDB spatial の `ST_Read` は非COI環境でスレッド生成に失敗しリモート URL も読めないため GPKG には使わない**。`duckdb:` で `.gpkg` 拡張子または `format=gpkg` を指定した場合も同関数へ振り分ける。ファイル全体をダウンロードするため大規模 GPKG は不向き）
